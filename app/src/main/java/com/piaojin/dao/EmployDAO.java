...
public class EmployDAO {
    ...
    public void save(Employ employ) {
        ...
        values.put("phoneip", employ.getPhoneip());
        values.put("pwd", "1");
    ...
    public Employ findEmploy(String name) {
        ...
        List<Employ> list=enclosure(result);
    ...
    private List<Employ> enclosure(Cursor result) {
        List<Employ> all = new ArrayList<Employ>();
        if (result.getCount() > 0) {
            for (result.moveToFirst(); !result.isAfterLast(); result.moveToNext()) {    // 采用循环的方式检索数据
                ...
                employ.setPhoneip(result.getString(result.getColumnIndex("phoneip")));
                all.add(employ);
            }
        }
        return all;
